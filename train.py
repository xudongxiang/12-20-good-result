...
def train(train_A_dir, train_B_dir, model_dir, model_name, random_seed, validation_A_dir, validation_B_dir, output_dir, tensorboard_log_dir):
    flag = 0 #1 表示新的数据 需要重新预处理   0 表示已经处理过，保存在特定位置
    ...
    if flag == 1:
        ...
        np.save(model_dir+'/wavs_A'+'.npy',wavs_A)
        wavs_B = load_wavs(wav_dir = train_B_dir, sr = sampling_rate)
        np.save(model_dir+'/wavs_B'+'.npy',wavs_B)
        ...
        print('----save f0s,timeaxes,sps,aps,coded_sps----')
        np.save(model_dir+'/f0s_A'+'.npy',f0s_A)
        np.save(model_dir+'/f0s_B'+'.npy',f0s_B)
        np.save(model_dir+'/timeaxes_A'+'.npy',timeaxes_A)
        np.save(model_dir+'/timeaxes_B'+'.npy',timeaxes_B)
        np.save(model_dir+'/sps_A'+'.npy',sps_A)
        np.save(model_dir+'/sps_B'+'.npy',sps_B)
        np.save(model_dir+'/aps_A'+'.npy',aps_A)
        np.save(model_dir+'/aps_B'+'.npy',aps_B)
        np.save(model_dir+'/coded_sps_A'+'.npy',coded_sps_A)
        np.save(model_dir+'/coded_sps_B'+'.npy',coded_sps_B)
        print('saved done')
        log_f0s_mean_A, log_f0s_std_A = logf0_statistics(f0s_A)
        log_f0s_mean_B, log_f0s_std_B = logf0_statistics(f0s_B)
        print('----save log_f0s_mean log_f0s_std----')
        print('Log Pitch A')
        print('Mean: %f, Std: %f' %(log_f0s_mean_A, log_f0s_std_A))
        print('Log Pitch B')
        print('Mean: %f, Std: %f' %(log_f0s_mean_B, log_f0s_std_B))
        np.save(model_dir+'/log_f0s_mean_A'+'.npy',log_f0s_mean_A)
        np.save(model_dir+'/log_f0s_mean_B'+'.npy',log_f0s_mean_B)
        np.save(model_dir+'/log_f0s_std_A'+'.npy',log_f0s_std_A)
        np.save(model_dir+'/log_f0s_std_B'+'.npy',log_f0s_std_B)
        print('saved done')

        coded_sps_A_transposed = transpose_in_list(lst = coded_sps_A)
        coded_sps_B_transposed = transpose_in_list(lst = coded_sps_B)
        print('----save coded_sps_transposed ----')
        #np.save(model_dir+'/coded_sps_A_transposed'+'.npy',coded_sps_A_transposed)
        #np.save(model_dir+'/coded_sps_B_transposed'+'.npy',coded_sps_B_transposed)
        print('save done')
        ...
        print('----save coded_sps_norm coded_sps_mean coded_sps_std ----')
        np.save(model_dir+'/coded_sps_A_mean'+'.npy',coded_sps_A_mean)
        np.savez(model_dir+'/coded_sps_A_norm.npz',coded_sps_A_norm[0],coded_sps_A_norm[1],coded_sps_A_norm[2],coded_sps_A_norm[3],coded_sps_A_norm[4],coded_sps_A_norm[5],coded_sps_A_norm[6],coded_sps_A_norm[7])
        #np.save(model_dir+'/coded_sps_A_norm'+'.npy',coded_sps_A_norm)
        np.save(model_dir+'/coded_sps_A_std'+'.npy',coded_sps_A_std)
        
        np.savez(model_dir+'/coded_sps_B_norm.npz',coded_sps_B_norm[0],coded_sps_B_norm[1],coded_sps_B_norm[2],coded_sps_B_norm[3],coded_sps_B_norm[4],coded_sps_B_norm[5],coded_sps_B_norm[6],coded_sps_B_norm[7])
        #np.save(model_dir+'/coded_sps_B_norm'+'.npy',coded_sps_B_norm)
        np.save(model_dir+'/coded_sps_B_mean'+'.npy',coded_sps_B_mean)
        np.save(model_dir+'/coded_sps_B_std'+'.npy',coded_sps_B_std)
        ...
        print('load data - wavs,f0s,timeaxes,sps,aps,coded_sps')
        wavs_A = np.load(model_dir+'/wavs_A'+'.npy')
        wavs_B = np.load(model_dir+'/wavs_B'+'.npy')
        f0s_A = np.load(model_dir+'/f0s_A'+'.npy')
        f0s_B = np.load(model_dir+'/f0s_B'+'.npy')
        timeaxes_A = np.load(model_dir+'/timeaxes_A'+'.npy')
        timeaxes_B = np.load(model_dir+'/timeaxes_B'+'.npy')
        sps_A = np.load(model_dir+'/sps_A'+'.npy')
        sps_B = np.load(model_dir+'/sps_B'+'.npy')
        aps_A = np.load(model_dir+'/aps_A'+'.npy',)
        aps_B = np.load(model_dir+'/aps_B'+'.npy')
        coded_sps_A = np.load(model_dir+'/coded_sps_A'+'.npy')
        coded_sps_B = np.load(model_dir+'/coded_sps_B'+'.npy')
        print('load data - log_f0s_mean,log_f0s_std')

        log_f0s_mean_A = np.load(model_dir+'/log_f0s_mean_A'+'.npy')
        log_f0s_mean_B = np.load(model_dir+'/log_f0s_mean_B'+'.npy')
        log_f0s_std_A = np.load(model_dir+'/log_f0s_std_A'+'.npy')
        log_f0s_std_B = np.load(model_dir+'/log_f0s_std_B'+'.npy')

        #coded_sps_A_transposed = np.load(model_dir+'/coded_sps_A_transposed'+'.npy')
        #coded_sps_B_transposed = np.load(model_dir+'/coded_sps_B_transposed'+'.npy')
        print('load data - coded_sps-norm-mean-std')
        print('change norm npy to npz')
        coded_sps_B_norm = []
        coded_sps_A_norm = []
        sps_A_norm = np.load(model_dir+'/coded_sps_A_norm'+'.npz')
        coded_sps_A_mean = np.load(model_dir+'/coded_sps_A_mean'+'.npy')
        coded_sps_A_std = np.load(model_dir+'/coded_sps_A_std'+'.npy')
        sps_B_norm = np.load(model_dir+'/coded_sps_B_norm'+'.npz')
        coded_sps_B_mean = np.load(model_dir+'/coded_sps_B_mean'+'.npy')
        coded_sps_B_std = np.load(model_dir+'/coded_sps_B_std'+'.npy')
        for i in range(8):
            new_s = 'arr_'+str(i)
            coded_sps_A_norm.append(sps_A_norm[new_s])
            coded_sps_B_norm.append(sps_B_norm[new_s])
