...
from utils import l1_loss, l2_loss, celoss
...
class CycleGAN(object):
    ...
    def build_model(self):
        ...
        # Generator loss
        # 生成器想要欺骗判别器
        self.generator_loss_A2B = celoss(tf.ones_like(self.discrimination_B_fake), self.discrimination_B_fake)
        self.generator_loss_B2A = celoss(tf.ones_like(self.discrimination_A_fake), self.discrimination_A_fake) 
        # Discriminator loss
        ...
        self.discriminator_loss_input_A_real = celoss(tf.ones_like(self.discrimination_input_A_real),  self.discrimination_input_A_real)
        self.discriminator_loss_input_A_fake = celoss(tf.zeros_like(self.discrimination_input_A_fake), self.discrimination_input_A_fake)
        self.discriminator_loss_A = self.discriminator_loss_input_A_fake + self.discriminator_loss_input_A_real

        self.discriminator_loss_input_B_real = celoss(tf.ones_like(self.discrimination_input_B_real), self.discrimination_input_B_real)
        self.discriminator_loss_input_B_fake = celoss(tf.zeros_like(self.discrimination_input_B_fake), self.discrimination_input_B_fake)
        ...
        self.Ag_loss = celoss(self.discrimination_A_fake,tf.ones_like(self.discrimination_A_fake))
        self.Bg_loss = celoss(self.discrimination_B_fake,tf.ones_like(self.discrimination_B_fake))
